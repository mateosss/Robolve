  ...
  ctor: function(hud, inventory) {
    ...
    for (let i = 0; i < this.slots.length; i++) {
      ...
      this.slots[i].itemThumb = new Badge({callback: () => { // jshint ignore:line
        let thumb = this.hud.inventory.grid.cells[ii].itemThumb; // TODO asco
        thumb.button.callback(thumb);
        if (!this.hud.inventory.inScreen) this.hud.inventory.show(true); // true is for force no refreshing for performance
